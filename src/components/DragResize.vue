<template>
  ...
  >
    <slot></slot>
    <div
      v-for="stick in sticks" :key="stick"
      class="vdr-stick"
...
function addEvents (events) {
  events.forEach((cb, eventName) => {
    this.$refs.vdr.addEventListener(eventName, cb)
...
function removeEvents (events) {
  events.forEach((cb, eventName) => {
    this.$refs.vdr.removeEventListener(eventName, cb)
...
export default {
  ...
  methods: {
    ...
    bodyDown (ev) {
      ...
      console.log('ev.stopPropagation', ev.stopPropagation)
